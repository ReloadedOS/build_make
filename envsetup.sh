...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the reloaded github
        T=$(gettop)
        cd $T > /dev/null
        vendor/reloaded/build/tools/roomservice.py $product
        cd - > /dev/null
        ...
        T=$(gettop)
        cd $T > /dev/null
        vendor/reloaded/build/tools/roomservice.py $product true
        cd - > /dev/null
    fi
